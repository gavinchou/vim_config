...
" Language: Markdown
" Filenames: *.md
" Creator: Gabriele Lana <gabriele.lana@gmail.com>
" Orignal: https://github.com/gabrielelana/vim-markdown/blob/master/syntax/markdown.vim
" Modified: Gavin Chou <gavineaglechou@gmail.com>
" Updated: 2018-11-09-Fri 19:07:06
...
for s:level in range(1, 16)
  ...
  execute 'syn region markdownListItemAtLevel' . (s:level) . ' '
    ...
    \ .   '@markdownInline '
    \ . 'start=/^' . (s:indented_as_container) . '\%([-*+]\|\d\+\.\|[a-zA-Z]\.\)\%(\s\+\[[ x]\]\)\?\s\+/ '
    \ . 'end='
    \ .   '/'
    \ .     '\n\%(\n\n\)\@='
    \ .     '\|'
    \ .     '\n\%(' . (s:indented_as_container) . '\%([-*+]\|\d\+\.\|[a-zA-Z]\.\)\s\+\)\@='
    \ .     '\|'
    \ .     '\n\%(\n' . (s:indented_as_container) . '\S\)\@='
    \ .   '/'
  " to hilight this region, uncomment the following
  " execute 'hi def link markdownListItemAtLevel' . (s:level) . ' Normal'
  ...
  execute 'hi def link markdownRuleInListItemAtLevel' . (s:level) . ' Title'
...
" {{{ EMOTICONS
" syn keyword does not support keyword with symbols like ':', maybe it's a bug
" syn-match should be used instead, however, it may cost to much and it will
" slow down the syntax highlighting, (so does the keyword thing?)

syn match markdownEmoticonKeyword ':bowtie:\|:smile:\|:laughing:\|:blush:\|:smiley:'
syn match markdownEmoticonKeyword ':relaxed:\|:smirk:\|:heart_eyes:\|:kissing_heart:\|:kissing_closed_eyes:'
...
hi def link markdownBold                    Reverse
hi def link markdownBoldItalic              Reverse

hi def link markdownPullRequestLinkInText   Underlined
hi def link markdownUserLinkInText          UrlLink
hi def link markdownUrlLinkInText           UrlLink
hi def link markdownEmailLinkInText         Purple

hi def link markdownLinkText                UrlLink
hi def link markdownLinkUrl                 Purple
hi def link markdownLinkTitleSingleQuoted   Reverse
hi def link markdownLinkTitleDoubleQuoted   Reverse
...
hi def link markdownInlineCode              Green
...
hi def link markdownBlockquote              Purple
...
hi def link markdownHeader                  Red
hi def link markdownH1                      Red
hi def link markdownH2                      YellowGreen
hi def link markdownH3                      Cyan
hi def link markdownH4                      Yellow
hi def link markdownH5                      Purple
hi def link markdownH6                      Blue

hi def link markdownEmoticonKeyword         Reverse
hi def link markdownRule                    Identifier

hi def link markdownXmlComment              Comment
